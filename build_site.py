...
.class-rank{position:absolute;bottom:10px;right:12px;font-size:10px;font-weight:600;color:rgba(180,170,155,.55);letter-spacing:.03em;pointer-events:none;z-index:2;line-height:1}
...
.profile-hero{border-radius:14px;overflow:hidden;margin-bottom:10px;padding:20px 18px 22px;background:linear-gradient(155deg,#1e1a16 0%,#0e0a08 72%);position:relative}
...
.stat-card{border-radius:14px;overflow:hidden;padding:16px 14px;background:linear-gradient(155deg,#1e1a16 0%,#0e0a08 72%);display:flex;flex-direction:column;gap:3px}
...
/* top-teachers list */
.teacher-list{display:flex;flex-direction:column;gap:10px;margin-top:10px}
.teacher-row{display:flex;align-items:center;gap:10px}
.teacher-rank{font-family:'DM Mono',monospace;font-size:10px;color:rgba(200,195,185,.35);width:14px;flex-shrink:0;text-align:center}
.teacher-avatar{width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:'DM Mono',monospace;font-size:11px;font-weight:600;color:#fff;flex-shrink:0}
.teacher-info{flex:1;min-width:0}
.teacher-name{font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;color:#f0ece4;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.teacher-bar-wrap{display:flex;align-items:center;gap:6px;margin-top:3px}
.teacher-bar-track{flex:1;height:3px;background:rgba(255,255,255,.1);border-radius:3px;overflow:hidden}
.teacher-bar-fill{height:100%;border-radius:3px}
.teacher-count{font-family:'DM Mono',monospace;font-size:10px;color:rgba(200,195,185,.4);flex-shrink:0}
/* monthly chart */
...
.seg-contemp{background:#5b9bd5}
.seg-other{background:rgba(200,195,185,.32)}
.chart-empty-bar{width:18px;height:2px;background:rgba(200,195,185,.1);border-radius:3px}
.month-label{font-family:'DM Mono',monospace;font-size:8px;color:rgba(200,195,185,.35);letter-spacing:.02em;white-space:nowrap}
.month-label.mcur{color:#f0a830}
.month-num{font-family:'DM Mono',monospace;font-size:9px;color:rgba(200,195,185,.5)}
.chart-toggle{display:flex;gap:2px;background:rgba(255,255,255,.07);border-radius:20px;padding:3px}
.ctb{font-family:'DM Mono',monospace;font-size:10px;color:rgba(200,195,185,.45);background:none;border:none;cursor:pointer;padding:4px 11px;border-radius:16px;letter-spacing:.04em;-webkit-tap-highlight-color:transparent}
.ctb.active{background:rgba(255,255,255,.13);color:#f5f1ea}
.chart-legend-row{display:flex;align-items:center;gap:6px;margin-top:10px;flex-wrap:wrap}
.cl-dot{width:7px;height:7px;border-radius:50%;display:inline-block;flex-shrink:0}
.cl-lbl{font-family:'DM Mono',monospace;font-size:9px;color:rgba(200,195,185,.38);letter-spacing:.03em;margin-right:8px}
.donut-wrap{display:flex;align-items:center;gap:18px;margin-top:12px}
.donut-svg{width:110px;height:110px;flex-shrink:0;overflow:visible}
.donut-center-text{font-family:'DM Mono',monospace;font-size:16px;font-weight:600;fill:#f5f1ea;text-anchor:middle;dominant-baseline:central}
...
.donut-legend-label{font-family:'DM Mono',monospace;font-size:10px;color:rgba(200,195,185,.6);flex:1;text-transform:capitalize;letter-spacing:.02em}
.donut-legend-count{font-family:'DM Mono',monospace;font-size:10px;color:rgba(200,195,185,.38)}
/* city bars */
.city-bar-row{display:flex;align-items:center;gap:8px;margin-bottom:8px}
.city-flag{font-size:14px;flex-shrink:0}
.city-name{font-family:'DM Mono',monospace;font-size:10px;color:rgba(200,195,185,.55);width:80px;flex-shrink:0;letter-spacing:.02em}
.city-bar-track{flex:1;height:6px;background:rgba(255,255,255,.1);border-radius:4px;overflow:hidden}
.city-bar-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,rgba(200,195,185,.5),rgba(200,195,185,.3))}
.city-bar-count{font-family:'DM Mono',monospace;font-size:10px;color:rgba(200,195,185,.45);width:22px;text-align:right;flex-shrink:0}
...
function renderMyClasses(){
  ...
  const stamped=[...scheduledStamped,...historicalStamped];
  if(!stamped.length){
  ...
  // ── chronological rank: sort all stamped earliest→latest, assign 1-based index ──
  const _rankSorted=[...stamped].sort((a,b)=>a.date_key<b.date_key?-1:a.date_key>b.date_key?1:(a.start_hour||0)-(b.start_hour||0));
  const _rankMap=new Map(_rankSorted.map((c,i)=>[classId(c),i+1]));
  ...
  function _appendCard(c,i){
    ...
    const rankNum=_rankMap.get(classId(c));
    if(rankNum!=null){const rk=document.createElement('span');rk.className='class-rank';rk.textContent='#'+rankNum;card.querySelector('.card-inner').appendChild(rk);}
...
const CITY_MAP={
  ...
  'Phresh NYC':'New York',
  'New York City Center':'New York','Pearl Studios':'New York',
...
function renderProfile(){
  ...
  // Collect all stamped (schedule + historical)
  const _schedStamped=[...ALL_CLASSES,...CUSTOM_CLASSES].filter(c=>isMyClass(c));
  const _schedIds=new Set(_schedStamped.map(c=>classId(c)));
  const _histStamped=Object.entries(myClassesMap)
    .filter(([id,v])=>!_schedIds.has(id)&&!v._wishlistOnly&&v.date_key)
    .map(([id,v])=>({...v,_historyId:id}));
  const stamped=[..._schedStamped,..._histStamped];
  const today=new Date();today.setHours(0,0,0,0);
  const upcoming=stamped.filter(c=>{const[y,m,d]=c.date_key.split('-').map(Number);return new Date(y,m-1,d)>=today;}).length;
  const past=stamped.length-upcoming;

  // ── Genre breakdown ──
  const genreCounts={};
  stamped.forEach(c=>{const g=c.genre||'other';genreCounts[g]=(genreCounts[g]||0)+1;});
  const topGenres=Object.entries(genreCounts).sort((a,b)=>b[1]-a[1]).slice(0,6);
  const maxG=topGenres[0]?topGenres[0][1]:1;

  // ── City breakdown ──
  const cityCounts={};
  stamped.forEach(c=>{const city=CITY_MAP[c.studio]||'Other';cityCounts[city]=(cityCounts[city]||0)+1;});
  ...
  const months=[];
  for(let i=17;i>=0;i--){
    let y=nowY,m=nowM-i;while(m<0){m+=12;y--;}
    const key=`${y}-${String(m+1).padStart(2,'0')}`;
    const pc=stamped.filter(c=>c.date_key&&c.date_key.slice(0,7)===key);
    const street=pc.filter(c=>['street','choreo'].includes(c.genre||'')).length;
    const contemp=pc.filter(c=>(c.genre||'')==='contemporary').length;
    months.push({label:MABB[m],count:pc.length,street,contemp,other:pc.length-street-contemp,isCurrent:i===0});
  }

  // ── Weekly activity (last 16 weeks) ──
  const weeks=[];
  for(let i=15;i>=0;i--){
    const wS=new Date(today);const dow=wS.getDay();
    wS.setDate(today.getDate()-(dow===0?6:dow-1)-i*7);wS.setHours(0,0,0,0);
    const wE=new Date(wS);wE.setDate(wS.getDate()+6);wE.setHours(23,59,59,999);
    const label=`${MABB[wS.getMonth()]} ${wS.getDate()}`;
    const pc=stamped.filter(c=>{if(!c.date_key)return false;const[y,m,d]=c.date_key.split('-').map(Number);const cd=new Date(y,m-1,d);return cd>=wS&&cd<=wE;});
    const street=pc.filter(c=>['street','choreo'].includes(c.genre||'')).length;
    const contemp=pc.filter(c=>(c.genre||'')==='contemporary').length;
    weeks.push({label,count:pc.length,street,contemp,other:pc.length-street-contemp,isCurrent:i===0});
  }
  ...
  function buildBarHTML(data){
    ...
    return rev.map(({label,count,street,contemp,other,isCurrent})=>{
      ...
      const oH=tH-sH-cH;
      return`<div class="month-col"><div class="month-num">${count}</div><div class="month-bar-space"><div class="chart-stack" style="height:${tH}px">${oH>0?`<div class="chart-seg seg-other" style="height:${oH}px"></div>`:''}${cH>0?`<div class="chart-seg seg-contemp" style="height:${cH}px"></div>`:''}${sH>0?`<div class="chart-seg seg-street" style="height:${sH}px"></div>`:''}</div></div><div class="month-label${isCurrent?' mcur':''}">${label}</div></div>`;
    }).join('');
  }

  // ── Milestone ──
  const milestones=[5,10,20,30,50,75,100,150,200,300];
  const next=milestones.find(m=>m>past)||null;
  const milestoneHTML=next?`<div class="stat-sub">${next-past} more to ${next} ✦</div>`:`<div class="stat-sub">Century dancer 🏆</div>`;
  ...
  const cityBarsHTML=topCities.map(([city,n])=>`
    <div class="city-bar-row">
      <span class="city-flag">${CITY_FLAG[city]||'🌍'}</span>
      <span class="city-name">${city}</span>
      <div class="city-bar-track"><div class="city-bar-fill" style="width:${Math.round(n/maxCity*100)}%"></div></div>
      <span class="city-bar-count">${n}</span>
    </div>`).join('');

  const teacherListHTML=top5Teachers.map(([name,n],i)=>{
    ...
    return`<div class="teacher-row">
      <span class="teacher-rank">${i+1}</span>
      <div class="teacher-avatar" style="background:${color}">${init}</div>
      <div class="teacher-info">
        <div class="teacher-name">${name}</div>
        <div class="teacher-bar-wrap">
          <div class="teacher-bar-track"><div class="teacher-bar-fill" style="width:${Math.round(n/maxT*100)}%;background:${color}"></div></div>
          <span class="teacher-count">${n}</span>
        </div>
      </div>
    </div>`;
  }).join('');

  // ── Donut chart (By Style) ──
  const donutTotal=topGenres.reduce((s,[,n])=>s+n,0);
  const circ=2*Math.PI*45;
  let cDash=0;
  const donutArcs=topGenres.map(([g,n])=>{
    const dash=(n/donutTotal)*circ,gap=circ-dash,offset=circ/4-cDash;
    cDash+=dash;
    return`<circle cx="60" cy="60" r="45" fill="none" stroke-width="18" stroke="${GENRE_COLORS[g]||'rgba(200,195,185,.35)'}" stroke-dasharray="${dash.toFixed(2)} ${gap.toFixed(2)}" stroke-dashoffset="${offset.toFixed(2)}"/>`;
  }).join('');
  const donutLegendHTML=topGenres.map(([g,n])=>`<div class="donut-legend-item"><span class="donut-legend-dot" style="background:${GENRE_COLORS[g]||'rgba(200,195,185,.35)'}"></span><span class="donut-legend-label">${GENRE_LABELS[g]||g}</span><span class="donut-legend-count">${n}</span></div>`).join('');
...
<div class="profile-pane">
  ...
  <div class="profile-grid">
    ${months.some(m=>m.count>0)||weeks.some(w=>w.count>0)?`<div class="stat-card full">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div class="stat-label">Activity</div>
        <div class="chart-toggle"><button class="ctb active" id="ctMonth">Month</button><button class="ctb" id="ctWeek">Week</button></div>
      </div>
      <div id="actChart" class="monthly-chart">${buildBarHTML(months)}</div>
      <div class="chart-legend-row"><span class="cl-dot" style="background:#f0a830"></span><span class="cl-lbl">Street / Choreo</span><span class="cl-dot" style="background:#5b9bd5"></span><span class="cl-lbl">Contemporary</span><span class="cl-dot" style="background:rgba(200,195,185,.35)"></span><span class="cl-lbl">Other</span></div>
    </div>`:''}
    ...
    ${topGenres.length&&donutTotal>0?`<div class="stat-card full">
      <div class="stat-label">By Style</div>
      <div class="donut-wrap">
        <svg class="donut-svg" viewBox="0 0 120 120">
          <circle cx="60" cy="60" r="45" fill="none" stroke-width="18" stroke="rgba(255,255,255,.07)"/>
          ${donutArcs}
          <text x="60" y="61" class="donut-center-text">${donutTotal}</text>
        </svg>
  ...
  // Wire chart toggle (after innerHTML set)
  const _ctM=listEl.querySelector('#ctMonth'),_ctW=listEl.querySelector('#ctWeek'),_ac=listEl.querySelector('#actChart');
  if(_ctM&&_ctW&&_ac){
    _ctM.addEventListener('click',()=>{_ctM.classList.add('active');_ctW.classList.remove('active');_ac.innerHTML=buildBarHTML(months);});
    _ctW.addEventListener('click',()=>{_ctW.classList.add('active');_ctM.classList.remove('active');_ac.innerHTML=buildBarHTML(weeks);});
  }
}
...
function openEditPopupDrawer(c){
  ...
  const classNames=[...new Set([...ALL_CLASSES,...CUSTOM_CLASSES].map(x=>x.class_name).filter(Boolean))].sort();
  scroll.innerHTML=`
<div style="display:flex;flex-direction:column;gap:14px;padding-bottom:24px">
  <datalist id="epd-classes">${classNames.map(n=>`<option value="${esc(n)}">`).join('')}</datalist>
  ...
  <button class="pmf-save" id="epdSaveBtn"><i class="ph ph-check-circle"></i> Save changes</button>
</div>`;
  ...
  scroll.querySelector('#epdSaveBtn').addEventListener('click',()=>{
    ...
    setTimeout(()=>{closeEditPopupDrawer();if(S.tab==='myclasses')renderMyClasses();else renderSaved();},900);
  });
