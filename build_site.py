...
/* monthly chart */
.monthly-chart{display:flex;align-items:flex-end;gap:4px;margin-top:14px;height:64px;overflow-x:auto;padding-bottom:18px;scrollbar-width:none}
...
.month-bar{width:18px;border-radius:3px 3px 0 0;background:rgba(200,195,185,.45);min-height:2px;transition:height .3s}
.month-bar.current{background:rgba(240,168,48,.7)}
...
function renderProfile(){
  ...
  stamped.forEach(c=>{
    ...
    const key=c.date_key.slice(0,7); // "YYYY-MM"
    monthCounts[key]=(monthCounts[key]||0)+1;
  });
  // Build array from earliest month to today
  ...
  for(let i=17;i>=0;i--){
    ...
    months.push({key,label,count:monthCounts[key]||0,isCurrent:i===0});
  }
  const maxMonth=Math.max(...months.map(m=>m.count),1);

  // ── Notes & misc ──
  let noteCount=0;Object.values(myClassesMap).forEach(v=>{if(v.notes)noteCount+=v.notes.length;});
  ...
  const milestoneHTML=next
    ?`<div class="stat-sub">${next-past} more to ${next} ✦</div>`
  ...
  const monthChartHTML=months.map(({label,count,isCurrent})=>{
    const barH=count>0?Math.max(Math.round(count/maxMonth*52),4):2;
    return`<div class="month-col">
      <div class="month-num" style="opacity:${count>0?1:0.3}">${count||''}</div>
      <div class="month-bar${isCurrent?' current':''}" style="height:${barH}px"></div>
