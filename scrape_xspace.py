#!/usr/bin/env python3
"""
X-Space Dance LIC — open class schedule scraper.

Data source: dance-schedule-unfold-studio.vercel.app/api/schedule
  (returns a pre-aggregated schedule; X-Space events have studioId = "xspace")

X-Space Dance uses the Mindbody Go platform.
Widget token (for reference): 0744235e061
Studio site:  https://www.xspacedance.com/
Booking URL:  https://go.mindbodyonline.com/book/widgets/schedules/view/0744235e061/schedule
MindBody site ID: 5751466
"""
...
OUTPUT_FILE = Path(__file__).parent / "xspace_schedule.json"
BOOKING_URL = "https://go.mindbodyonline.com/book/widgets/schedules/view/0744235e061/schedule"
...
def parse_xspace_classes(data: dict) -> list:
    events = data.get("events", [])
    xspace_events = [e for e in events if e.get("studioId") == "xspace"]

    out = []
    for e in xspace_events:
        ...
        out.append({
            ...
            "source_url":         "https://www.xspacedance.com/",
            "studio":             "X-Space Dance",
...
def main():
    print("Fetching X-Space Dance schedule…")
    try:
        data = fetch_schedule()
    ...
    classes = parse_xspace_classes(data)
    ...
    output = {
        ...
        "mindbody_widget_token": "0744235e061",
        "mindbody_site_id":      "5751466",
